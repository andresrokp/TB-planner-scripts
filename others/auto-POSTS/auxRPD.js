...
async function deleteTelemetry(entityType, entityId, keys, deleteAllDataForKeys, startTs, endTs, rewriteLatestIfDeleted, authorizationToken) {
  const url = `https://api.sighums.com/api/plugins/telemetry/${entityType}/${entityId}/timeseries/delete?keys=${encodeURIComponent(keys)}&deleteAllDataForKeys=${deleteAllDataForKeys}&rewriteLatestIfDeleted=${rewriteLatestIfDeleted}`;
  const headers = {
