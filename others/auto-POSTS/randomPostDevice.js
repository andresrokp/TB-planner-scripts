...
async function sendMessageToDevice(deviceId, message) {
  ...
  try {
    ...
    log(`${counter++} :: Message sent to device ${message.device.uniqueId} - ${message.device.name}`); // response
  } catch (error) {
    console.error(`${counter++} :: Error sending message to device ${deviceId}:`);
  }
}
...
// to launch load test
Array(100).fill().forEach(()=>mainALoCerdo()) //
