...
self.onInit = async function () {
  ...
  const actualExpectedTest =
    self.ctx.stateController.dashboardId ==
      "c7c456b0-b450-11ed-9b83-e14509358390" ||
    self.ctx.stateController.dashboardId ==
      "3cb087c0-c859-11ed-9b83-e14509358390";
  ...
  for (let [device, deviceData] of Object.entries(mainStore)) {
    ...
    for (let i = 0; i < regNum.length; i++) {
      ...
      if (actualExpectedTest && blockin[i] && pushback[i]) {
        ...
        let operationDiffMinutes =
          Math.round((operationDiffMillis / 1000 / 60) * 10) / 10;
        items.add({
          id: ++j,
          group: device,
          className: "operation",
  ...
  function getNextEventStart(groupId) {
    var itemsOfGroup = items.get({
      filter: function (item) {
        return item.group == groupId;
      },
    });
    var now = new Date();
    var upcomingEvents = itemsOfGroup.filter(function (item) {
  ...
  // create and set up the Timeline
  var container = document.getElementById("visualization");
  timeline = new vis.Timeline(container, null, options);
  timeline.setGroups(groups);
  timeline.setItems(items);
  timeline.on("doubleClick", function (event) {
    // structure of event and items.get(event.item) at the end
    // event.item is an integer with value equal to id @ .add
  ...
  function telemetryLoadPushAndGo(stateId, _entityId, entityName, ts_id) {
    ...
    attributeService
      .getEntityTimeseries(entityId, keys, ts_id - 2, ts_id + 2)
    ...
    function updateAttributes(attributesArray) {
      attributeService
        .saveEntityAttributes(entityId, "SHARED_SCOPE", attributesArray)
        .subscribe(function () {
          openDashboardState();
